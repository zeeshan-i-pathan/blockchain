...
impl App {
    ...
    fn try_add_block(&mut self, block: Block) {
        let latest_block = self
            .blocks
            .last()
            .expect("There should be atleast one block");
...
fn mine_block(id: u64, timestamp: i64, previous_hash: &str, data: &str) -> (u64, String) {
    ...
    loop {
        ...
        if binary_hash.starts_with(DIFFICULTY_PREFIX) {
            info!(
                "mined! nonce: {nonce}, hash: {}, binary hash: {binary_hash}",
                hex::encode(&hash)
            );
...
fn hash_to_binary_representation(expect: &[u8]) -> String {
...
async fn main() {
    ...
    let (init_sender, mut int_rcv) = mpsc::unbounded_channel();
    ...
    loop {
        let evt: Option<p2p::EventType> = {
            select! {
                line = stdin.next_line() => Some(p2p::EventType::Input(line.expect("cen get line").expect("can read line from stdin"))),
